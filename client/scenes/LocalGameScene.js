...
export default class LocalGameScene extends Phaser.Scene {
  ...
  createBigUpgradesPanelToolbar() {
    ...
    if (canGoPrevPage) {
      this.bigPagePrevBtn = this.add.text(startX - 22, this.scale.height * 0.5, '◀', { fontSize: 28, fontFamily: 'Orbitron, Arial', color: '#ffffff' })
        .setDepth(1003).setOrigin(0.5).setVisible(false).setInteractive({ useHandCursor: true }).on('pointerdown', () => this.goToPreviousBigUpgradePage());
    }
    if (canGoNextPage) {
      this.bigPageNextBtn = this.add.text(this.scale.width - 24, this.scale.height * 0.5, '▶', { fontSize: 28, fontFamily: 'Orbitron, Arial', color: '#ffffff' })
        .setDepth(1003).setOrigin(0.5).setVisible(false).setInteractive({ useHandCursor: true }).on('pointerdown', () => this.goToNextBigUpgradePage());
    }
    this.bigUpgradesOpen = false;
    this.updateBigUpgradePageArrowVisibility();
  }
  ...
  updateBigUpgradePageArrowVisibility() {
    const show = !!this.bigUpgradesOpen;
    if (this.bigPagePrevBtn) this.bigPagePrevBtn.setVisible(show);
    if (this.bigPageNextBtn) this.bigPageNextBtn.setVisible(show);
  }
